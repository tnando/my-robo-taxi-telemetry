...
// TripActivityToken is one registered push-to-start token.
type TripActivityToken struct {
	TripID string
	UserID string
	// Token is the raw ActivityKit push-to-start token. P1 — never log in full.
	Token string
	// Sandbox selects the APNs sandbox gateway. Carried PER REGISTRATION rather
	// than read from the device registry because starting an Activity needs no
	// notification permission, so the user may have no go_push_devices row at
	// all (§7.21's own reasoning for the same field).
	Sandbox bool
}

// queryUpsertTripActivityToken registers or replaces a push-to-start token.
//
// The conflict target is (trip_id, user_id) rather than the token, because
// ActivityKit ROTATES the push-to-start token: keying on it would accumulate a
// row per rotation and leave the sender guessing which one is live. Keying on
// the pair makes a rotation an ordinary re-registration.
//
// UNGUARDED, unlike go_live_activities' upsert, and the asymmetry is the point.
// That statement refuses a registration on a ride that has reached a terminal
// status, because an update token addresses a card that must not be resurrected.
// A push-to-start token addresses no card at all — it is a standing permission
// slip for the next leg — so there is nothing to resurrect, and refusing it
// mid-trip would silently stop the trip's remaining legs from ever raising an
// Activity. Registration on a trip that has ENDED is harmless: the leg detector
// only ever looks at open windows, so the row is simply never used, and account
// deletion and the trip's own CASCADE clean it up.
...
INSERT INTO go_trip_activity_tokens (trip_id, user_id, push_to_start_token, sandbox, created_at, updated_at)
...
// queryDeleteTripActivityToken removes the caller's own registration —
// the DELETE half of the §7.30 endpoint pair.
// #nosec G101 -- column/predicate SQL over the push-to-start registry, not a
// credential literal (gosec greps the identifier 'token' in a string).
...
func (r *TripActivityTokenRepo) PushToStartTokensForTrip(ctx context.Context, tripID string) ([]TripActivityToken, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tok.TripID, &tok.UserID, &tok.Token, &tok.Sandbox); err != nil {
