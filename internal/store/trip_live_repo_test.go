...
func seedTrip(t *testing.T, id, vehicleID, ownerID string, startsAt, endsAt time.Time) {
	t.Helper()
...
func TestTripClaims_AreOneShot(t *testing.T) {
	...
	now := time.Now().UTC()
	seedTrip(t, "ctrip0602open", "cveh0602", "cowner0602",
...
func TestClaimTripStartNow_RefusesAnUnopenedWindow(t *testing.T) {
	...
	now := time.Now().UTC()
	seedTrip(t, "ctrip0602future", "cveh0602f", "cowner0602",
...
func TestTripAudienceFor_ExcludesDepartedAndSuspended(t *testing.T) {
	...
	now := time.Now().UTC()
	seedTrip(t, tripID, vehicleID, "cowner0602aud", now.Add(-time.Hour), now.Add(time.Hour))
...
func TestStartLeg_OneOpenLegPerTrip(t *testing.T) {
	...
	now := time.Now().UTC()
	seedTrip(t, tripID, vehicleID, "cowner0602leg", now.Add(-time.Hour), now.Add(time.Hour))
...
func TestLegClaims_AreOneShot(t *testing.T) {
	...
	now := time.Now().UTC()
	seedTrip(t, tripID, vehicleID, "cowner0602claims", now.Add(-time.Hour), now.Add(time.Hour))
	if _, err := testPool.Exec(ctx, `DELETE FROM go_trip_legs WHERE trip_id = $1`, tripID); err != nil {
		t.Fatalf("clean legs: %v", err)
	}
...
func TestPushToStartTokens_UpsertAndReject(t *testing.T) {
	...
	now := time.Now().UTC()
	seedTrip(t, tripID, "cveh0602tok", "cowner0602tok", now.Add(-time.Hour), now.Add(time.Hour))
	...
	if tokens[0].Token != "pts-v2" || tokens[0].Sandbox {
