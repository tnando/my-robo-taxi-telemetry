...
// TripLeg is one driving leg.
type TripLeg struct {
	ID        string
	TripID    string
	VehicleID string
	// DestinationName is the decrypted place name. Empty means the ciphertext
	// would not decrypt — fail-soft, exactly as every other label read in this
	// package, and the copy and content-state both have a no-destination
	// fallback for it.
	DestinationName string
	StartedAt       time.Time
	EndedAt         *time.Time
	Arrived         bool
	// StartedNotifiedAt / ArrivedNotifiedAt are the per-leg push stamps.
	StartedNotifiedAt *time.Time
	ArrivedNotifiedAt *time.Time
	// ActivityStartedAt / ActivityEndedAt record the push-to-start fan-out and
	// the end fan-out. SEPARATE from the push stamps above because they are
	// separate deliveries with separate failure modes: an alert can succeed
	// while a push-to-start fails, and each must retry without re-sending the
	// other.
