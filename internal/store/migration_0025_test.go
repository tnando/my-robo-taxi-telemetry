...
var migration0025Columns = map[string]string{
	...
	// keyed to the thing it is ABOUT, and a trip LEG is the second kind of
	// thing after a ride; the migration's CHECK makes exactly one of
	// `ride_request_id` and this set, which is why `ride_request_id` lost its
	// NOT NULL in the same statement. P0: an opaque cuid.
	"trip_leg_id": "text",
}
