...
## 13. A drive says which trip it belongs to, and a trip says what it adds up to (MYR-608)

**The problem was two implementations of one rule.** After [MYR-605](https://linear.app/myrobotaxi/issue/MYR-605) the client grouped a drive list under its trips by **re-deriving the window predicate** — `drive.startedAt ∈ [startsAt, effectiveEnd]` — and, because it could only sum the page in hand, **withheld a total (`"— mi"`) whenever that page was smaller than `driveCount`**. So the one number a rider's collapsed trip header most wanted was the one it could never state, and §5's window rule now had a second author who would not be told when it changed.

Both halves move to the server. Neither needs a migration, and that is the design rather than a saving.

### 13.1 `tripId` is a JOIN, not a column

**Nothing tags a drive with a trip at write time, and nothing may start.** That absence is what makes §2's third window rule work: a window opened on Tuesday for a drive that began on Sunday picks up the legs already driven, with **no backfill and no write at drive time**. A `trip_id` column on `"Drive"` would be a second source of truth that a back-dated or extended window immediately falsifies — and `"Drive"` is Prisma-owned, so CG-DL-9 forbids adding one anyway. The membership question is therefore answered **at read time, in the statement that produces the row**.

**It is answered THREE WAYS, because the role is three different questions.** Collapsing them into one expression would be exactly the failure §5's invariant 3 exists to prevent:

| Surface and caller | The expression | Why it is this and not the others |
|---|---|---|
| §7.2, `owner` | `ownerTripIDForDriveExpr` — the caller's OWN windows on that vehicle | **A car can change hands.** A drive inside the PREVIOUS owner's window must resolve `null`: naming it would say a stranger's trip exists and hand over its id, on a surface where every per-trip route answers `404` to anybody not on the trip |
...
- **§7.2 breaks the tie NEWEST-FIRST** (`ORDER BY t.starts_at DESC, t.id DESC`), decided in the statement rather than left to the planner.
- **§7.30.7 always answers with the trip it was asked for.** A row naming a different overlapping trip would make the page disagree with the route that produced it, and a client grouping by `tripId` would draw a section for a trip it never asked about.

This is the **only** place on the platform where two surfaces report a different `tripId` for one drive, and it is documented on both of them.
...
`totalDistanceMiles` and `totalDurationSeconds` come from **widening the statement that already computed `driveCount`**: one `SELECT COUNT(*), SUM("distanceMiles"), SUM("durationMinutes")` where there was one `SELECT COUNT(*)`.

**Two reasons, and the second is the binding one.** The cheap one is cost: §7.30.2 decorates **every row it returns** (§7.30.2's own note), so a separate sum would have added one round trip *per trip* to a list whose cost is already `1 + N × decoration` — invisible on a list of one, a fifth of the round trips on a list of twenty, with nothing in the response to show for it. The load-bearing one is **agreement**: a count and a total read by two statements could straddle a drive being written and describe two different sets of drives on one card, with no way for a reader to tell.

`TestTripListDecorationIssuesNoExtraQueryPerTrip` pins it by counting **statements** through a pgx `QueryTracer` over a private single-connection pool — the repository's own metrics count repository CALLS, and a counter that cannot tell "one call issuing five queries" from "one call issuing four" would pass either way.
...
| The mask allow-list, and why the mask is the wrong instrument here | `internal/mask/tables.go` |
...
⚠ **UNTIL A PUSH SWEEP RUNS, THE RESEND IS DORMANT.** Every car linked before this change keeps its stored config, so the 300s resend reaches nobody until an `ops fleet-config push` sweep re-pushes the fleet — filed as **[MYR-630](https://linear.app/myrobotaxi/issue/MYR-630)** rather than run inside this change, because it costs ~288 extra frames per parked car per day and is an operational decision, not a code one.
...
**A NET-REGEN DRIVE REPORTS ITS NEGATIVE**, and the review round corrected this. The physical figure is negative and real; clamping it to 0 per drive manufactured the exact value §14.4 reads as *"this drive was never measured"*, so a real measurement of a downhill leg voided the whole window's total — the clamp defeating the rule it existed to protect. The floor belongs on the WINDOW, applied once after the legs are added up, which is where it now is: a negative leg still cannot cancel another leg's consumption, and `DriveDetail.energyUsedKwh` lost its `minimum: 0` in the OpenAPI spec to say so (a widening, carried by contracts v0.45.0).

**AN UNMEASURABLE DRIVE WRITES 0**, because the column is `NOT NULL`. What makes that zero honest downstream is §14.4. ⚠ Note that `0` therefore carries two meanings — *nothing could be measured* and *the drive genuinely consumed nothing* (a credited step of exactly zero) — and the veto reads both as the first. The second is vanishingly rare on a drive that covered distance, which is the only case the veto looks at.
...
CASE
     ...
     WHEN COUNT(*) FILTER (WHERE d."distanceMiles" > 0 AND d."energyUsedKwh" = 0) > 0 THEN NULL
     ELSE GREATEST(SUM(d."energyUsedKwh") FILTER (WHERE d."distanceMiles" > 0), 0)
END
```
...
Three details of that statement are the review round's, and each one is load-bearing:

- **The unmeasured test is `= 0`, not `<= 0`.** A negative row is a net-regen leg — a real measurement (§14.3) — and `<= 0` would void a window because one of its legs went downhill.
- **The sum is FILTERed to the rows that moved**, which is the same set the veto judges. `Drive` is Prisma-owned and the Next.js app writes it too; nothing constrains a 0-mile row to carry 0 energy, and such a row is exempt from the veto, so it must be exempt from the sum. One set of rows decides the total and vetoes it.
- **`GREATEST(..., 0)` floors the window once**, after the legs are added up — the arithmetic a per-leg clamp cannot express. ⚠ Postgres' `GREATEST` **ignores NULLs** (`GREATEST(NULL, 0)` is `0`), which is why the two null answers are spelled as their own `WHEN` arms above it rather than left to fall out of an empty `SUM`. An empty window is `null`; a window whose drives all went nowhere is `0`.

**A DRIVE THE RECONCILER CLOSED IS THE ONE ROW THIS RULE CANNOT SEE.** When the server restarts mid-drive, `MarkOrphanEnded` (`cmd/telemetry-server/adapters.go`) closes the row with `DriveCompletion{EndTime: now}` and **zero everything** — the in-memory state that held the distance and the accumulator was lost with the process. So the row lands at `distanceMiles = 0`, which is precisely the shape the qualifier spares: it does not veto, and its (also zero) energy is now filtered out of the sum. A trip window containing a restart therefore reports an energy total covering **fewer miles than the window actually drove**, with nothing on the wire to say so. It is the same hole the distance and duration totals have carried since MYR-608, it needs a real fix in the reconciler rather than a patch in this statement, and it is filed as **[MYR-631](https://linear.app/myrobotaxi/issue/MYR-631)**.
