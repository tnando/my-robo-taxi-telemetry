...
>
> **The GAIN direction is now propagated too (MYR-609).** A client that becomes entitled to MORE than its handshake snapshot says used to see the wider set only after reconnecting on its own. That direction fails safe — the client is shown less than it could be, never more — and had no live producer, so it was catalogued rather than fixed. §7.5.8 share extend is the first producer, and it is handled by §4.5.2 below with the same mechanism and the same close frame. The remaining residual is a ROLE upgrade in place (`viewer` → `owner`), which still has no producer. See §10 DV-09.
...
**[MYR-601](https://linear.app/myrobotaxi/issue/MYR-601) is the same class** — a viewer→owner transfer widens what a live session may see — and should use this pipeline rather than inventing a second one.
