...
**Atomic groups that do NOT map to a dedicated wire accumulator:** `charge`, `gps`, and `gear`. The server delivers these groups' fields in one `vehicle_update` **iff** Tesla emits the members within a single upstream telemetry batch (`VehicleTelemetryEvent.Fields`) and none of them are nav fields. Tesla's emission model (value-change + interval, see the [`tesla-fleet-telemetry-sme`](../../.claude/skills/tesla-fleet-telemetry-sme/) skill) typically batches `{latitude, longitude, heading}` and `{chargeLevel, estimatedRange}` together, which satisfies NFR-3.1 for those groups without an extra server-side debounce. Gear updates are typically single-field (`gearPosition`) with `status` derived at broadcast time -- see §4.1.5.
...
**This is NOT a server-side design choice -- it is a Tesla-side floor.** The Tesla fleet telemetry emission model batches field changes in **500 ms buckets on the vehicle side**: multiple field changes that occur within the same 500 ms window are already coalesced into a single upstream message before the server sees them. A shorter server-side accumulator window (e.g., the 200 ms literal in the current NFR-3.2 text) would fire before straggler fields from the SAME logical update have arrived, causing exactly the half-updated UI race that the accumulator exists to prevent. Additionally, Tesla's Fleet API `interval_seconds` parameter is an integer with a **minimum of 1 second** ([`internal/telemetry/fleet_api_fields.go`](../../internal/telemetry/fleet_api_fields.go) lines 112-118), so sub-second emission cannot even be requested -- the 500 ms bucket is the absolute floor. See the [`tesla-fleet-telemetry-sme`](../../.claude/skills/tesla-fleet-telemetry-sme/) skill's `architecture-and-setup.md` §"Emission model" for the authoritative reference.
...
| `timeToFull` | `number` | seconds | proto field **43** (`TimeToFullCharge`, `double`) | Seconds to full charge at the current rate. `0` (or `null` when disconnected) means no active charging session. Tesla proto type is `double`, so the wire value is a JSON number (NOT rounded to integer). |

**Server flow:** Mapped from Tesla's native fields above. Tesla emits these on the shared 30-second charge cadence; when any of the four fields change the broadcaster delivers the full group in the same `vehicle_update` because they arrive in the same `VehicleTelemetryEvent.Fields` batch (same Tesla 500 ms bucket). No additional server-side coalescing is required beyond the §3.2.1 atomic-group rule.
...
| **DV-04** | **RESOLVED (target documented; wiring still pending)** | `timeToFull` in charge group | Not emitted, not persisted. Previous `vehicle-state-schema.md` §7.1 entry claimed this field was "not available from Tesla Fleet Telemetry" -- **this claim was factually wrong.** Tesla emits `TimeToFullCharge` natively as proto field **43** (`double`, seconds). | **v1 ships `timeToFull` as a member of the `charge` atomic group.** The follow-up implementation issue can be bundled with DV-03's (same pipeline changes). Corrected the incorrect claim in `vehicle-state-schema.md` line 333 as part of this PR. See §4.1.4 for the full wire contract. | NFR-3.1, §4.1.4 | `MYR-XX Implement timeToFull wire field (v1 charge atomic group)` |
