...
| **NFR-3.21** | Vehicle ownership enforced on every subscription | §2.2 vehicle resolution; §4.5 ownership filtering; §4.5.1 mid-connection revocation (DV-09 closed for access loss) |
...
>
> **The residual is capability GAIN, not loss.** A client that becomes entitled to MORE than its handshake snapshot says — the role-upgrade case this note used to describe — still sees the wider set only after reconnecting. That direction fails safe (the client is shown less than it could be, never more) and has no live producer today. See §10 DV-09.
...
**What is NOT covered.** A grant being *widened* (a capability gained) still needs a reconnect; that direction shows the client less than it is entitled to, never more. `allowRides` has no WebSocket effect at all and deliberately does not close anything — it governs the §7.8 ride surface, so tearing down a live map over it would be a disconnection for a change that does not touch the live map.
...
| `4002` | Permission Revoked | Access to a vehicle ended while connected. Three producers, one indistinguishable frame: the Vehicle row was deleted (MYR-73), the caller's share grant was **revoked**, or it was **suspended** (MYR-373, §4.5.1). The server deliberately does not say which — a viewer is not told whether the car went away or the owner cut them off. | **Reconnect, then render whatever the new handshake returns.** The reconnect is the refresh: it re-derives the access set and the per-vehicle roles, so a caller who lost one of several vehicles comes back with the rest. Do NOT auto-retry a `subscribe` for the specific vehicleId that was open — if it is still in the reduced set the handshake covers it, and if it is not, retrying is a loop. |
