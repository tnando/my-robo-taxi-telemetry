...
| **NFR-3.21** | Vehicle ownership enforced on every subscription | §2.2 vehicle resolution; §4.5 ownership filtering; §4.5.1 mid-connection revocation (DV-09 closed for access loss); §4.5.2 / §4.5.3 mid-connection widening, incl. the sweep's widen backstop (DV-09 closed for access gain) |
...
>
> **The GAIN direction is now propagated too (MYR-609, MYR-601).** A client that becomes entitled to MORE than its handshake snapshot says used to see the wider set only after reconnecting on its own. That direction fails safe — the client is shown less than it could be, never more — and had no live producer, so it was catalogued rather than fixed. §7.5.8 share extend was the first producer (§4.5.2); MYR-601 then found that it was never the only one, and wired the remaining five — owner provisioning, the owner-wins transfer, §7.5.5 redeem, an un-suspending §7.5.4 `PATCH` and §7.24 ride join — to the same signal (§4.5.3). Every event that adds a vehicle to somebody's access set now announces it, **and the 60s revalidation sweep backstops the gain direction too since MYR-601** — which matters because one widening writer is outside this process entirely (the Next.js app's direct `"Vehicle"` insert) and can publish nothing. See §10 DV-09.
...
**[MYR-601](https://linear.app/myrobotaxi/issue/MYR-601) turned out to be the same class and five producers wide** — see §4.5.3, which reuses this pipeline exactly as this paragraph asked the next producer to.


#### 4.5.3 The widening rule, its five producers and its backstop (MYR-601)
...
|---|---|---|---|
| Owner provisioning (in-app Tesla link, deliberate re-add) | `cmd/telemetry-server/owner_stream_hook.go` | `provisioned` | the upsert **created** the row (`xmax = 0`), and **once per pass, not once per car**. A passive re-link that merely reconciles a car the user has had for months announces nothing — otherwise every Tesla re-consent would re-handshake every session that owner holds — and a first link of an N-car fleet announces ONCE, because the re-handshake re-derives the whole access set and the other N−1 closes would only race the reconnect the first one provoked. Same rule as redeem's one-signal-per-redemption, arriving from the other side. |
| Owner-wins transfer (MYR-599) | same hook | `owner_transfer` (gain) + `superseded_by_owner` (the linker's loss) + `share_superseded_by_owner` (each grantee's loss), the losses on `share.access_revoked` | the car moved. **Every loser is named, and the losses are published FIRST.** The arriving owner gains the car; the former driver loses it; and so does **every third party whose share the same transaction tombstoned** — `queryRevokeSharesForVehicle` revokes every live grant on the vehicle, so the driver's viewers, who never linked anything, are cut in the same statement and are the sessions most likely to be open. `store.VehicleUpsertResult` carries `PreviousUserID` and `RevokedGranteeIDs` out of the transaction; a pending invite's NULL grantee is not one. |
...
| A writer in ANOTHER PROCESS — the Next.js app inserts `"Vehicle"` rows directly (`react-frontend` `src/features/vehicles/api/sync.ts`), reaching neither this process's access cache nor its bus | **≤ 60s + the 5-minute access-cache TTL**, because the sweep's own resolver reads through that cache and no bust ever happened for it. Bounded, not fast; nothing in this process can publish for a writer outside it |
