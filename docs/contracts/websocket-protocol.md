...
**Implementation status:** wire delivery is live as of MYR-40; REST `/snapshot` persistence (DB columns for `chargeState` + `timeToFull` on the Prisma-owned `Vehicle` table) is tracked as a follow-up issue requiring cross-repo coordination with `../my-robo-taxi`. Until that lands, the cold-start snapshot returns `null` for the two fields while the live WS stream delivers real values.

> **SDK MUST** continue to tolerate `null` for `chargeState` and `timeToFull` on REST `/snapshot` responses until the DB-persistence follow-up ships (see §10 DV-03 / DV-04 status). Live WS frames emitted post-MYR-40 will carry real values whenever Tesla emits them; pre-MYR-40 frames cached by older SDK builds may still be null.

> **Generated SDK types.** `chargeState` and `timeToFull` MUST remain nullable in generated SDK types (`string | null` in TypeScript, `String?` / `Optional<String>` in Swift, `Double?` for `timeToFull` in Swift) until the DB persistence follow-up lands and the REST snapshot reliably carries values. Tighten to non-null in a follow-up generator run after that.
>
> **Consumer UI.** SDK consumers rendering charge state SHOULD surface `null` as a neutral placeholder (`--`, `—`, or hide the field entirely), NOT as a loading spinner. With `dataState.charge = ready` and `chargeState: null`, the steady-state meaning is "no data yet from Tesla for this field," NOT "loading in progress." A spinner implies work is in flight, which is misleading during the pre-DB-persistence window. A time-to-full display might read `— minutes remaining` until the REST snapshot carries `timeToFull`; a charge-state badge might hide until `chargeState` becomes non-null.
...
| **DV-03** | **RESOLVED (wire wiring live; REST snapshot DB persistence pending)** | `chargeState` in charge group | Live WS wiring landed in [MYR-40](https://linear.app/myrobotaxi/issue/MYR-40) (2026-04-22): `FleetFieldChargeState` added to `DefaultFieldConfig`, `convertChargeState` routes proto 2's `Value_ChargingValue` oneof through `chargingStateString`, `Field_ChargeState` is in `fieldMap` with the internal name `chargeState`. The wire field passes through `internal/ws/field_mapping.go` unchanged (internal == wire name). REST `/snapshot` still returns `null` because the Prisma-owned `Vehicle` table does not yet have a `chargeState` column. | **v1 ships `chargeState` as a member of the `charge` atomic group.** Tesla emits `ChargeState` natively as proto field **2** (enum: `Unknown`, `Disconnected`, `NoPower`, `Starting`, `Charging`, `Complete`, `Stopped`). The remaining DB-persistence follow-up requires a cross-repo PR in `../my-robo-taxi` (add `chargeState String?` to `Vehicle` in `prisma/schema.prisma` + `prisma migrate deploy` on prod), then a Go-side PR updating `internal/store` to read the column. See §4.1.4 for the full wire contract. | NFR-3.1, §4.1.4 | `MYR-XX Persist chargeState + timeToFull to Prisma Vehicle table (cross-repo)` |
| **DV-04** | **RESOLVED (wire wiring live; REST snapshot DB persistence pending)** | `timeToFull` in charge group | Live WS wiring landed in [MYR-40](https://linear.app/myrobotaxi/issue/MYR-40) (2026-04-22): `FleetFieldTimeToFullCharge` added to `DefaultFieldConfig` at 30s, `Field_TimeToFullCharge` is in `fieldMap` with internal name `timeToFull`, `convertNumericOrString` handles the `DoubleValue` hours passthrough. The empirical unit verification DV-17 is closed (see the [MYR-25 comment](https://linear.app/myrobotaxi/issue/MYR-25#comment-4f1dcee9-ab10-4039-acc5-9e7ef25c3762) — 1.0667h observed at chargeLevel 68 during home L2 charging on 2026-04-22). REST `/snapshot` still returns `null` because the Prisma-owned `Vehicle` table does not yet have a `timeToFull` column. | **v1 ships `timeToFull` as a member of the `charge` atomic group with unit `hours` (decimal double).** The remaining DB-persistence follow-up is bundled with DV-03's (cross-repo Prisma PR in `../my-robo-taxi` adding `timeToFull Float?` to `Vehicle`). See §4.1.4 for the full wire contract. Note that Tesla also exposes `EstimatedHoursToChargeTermination` (proto field 190) as a related "simple" ETA that always targets `ChargeLimitSoc`; proto 43 is the v1 source because it is trip-aware (time-to-trip-ready during Trip Planner sessions) -- delineation and decision rationale resolved by [MYR-28](https://linear.app/myrobotaxi/issue/MYR-28) on 2026-04-21, see `vehicle-state-schema.md` §7.1 for the full comparison and citations. Proto 190 stays held out of `fieldMap` pending the MYR-28 flip-condition Trip Planner capture tracked in MYR-25. | NFR-3.1, §4.1.4 | `MYR-XX Persist chargeState + timeToFull to Prisma Vehicle table (cross-repo)` |
