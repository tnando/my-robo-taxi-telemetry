...
> **Known gap (out of MYR-14 scope, tracked separately).** When a telemetry event arrives carrying only `speed` (no `gear`), `status` is NOT recomputed in `handleTelemetry`. A vehicle that transitions from "parked, speed 0" to "speed > 0 with no fresh gear update" therefore stays at `status: parked` until the next `gear` event. Recomputing on speed-only frames would emit a `status` field unsynchronized with `gearPosition`, which would be a worse atomicity violation. The fix needs to source `gearPosition` from accumulator state or DB read so `status` always rides alongside its companion. Tracked as a follow-on Linear issue with `relatedTo` MYR-14.
