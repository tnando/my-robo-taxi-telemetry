...
| [`schemas/ride-request.schema.json`](schemas/ride-request.schema.json) | [`rest-api.md`](rest-api.md) §7.8, §7.24 | The P10 ride-hailing shapes: `RideRequest` and its lifecycle enums, the create/trip-patch request bodies, the multi-stop `RideStop` pair (v0.34.0, MYR-539) and the group-ride `RideMember` + `RideRequestJoinRequest` pair (v0.35.0, MYR-540). **Vendored here late:** PRs #41 and #42 authored v0.34.0 and v0.35.0 directly in `myrobotaxi/contracts` and skipped this authoring home, which they flagged; MYR-540 closes that debt for both versions at once by landing the current file. |
| [`schemas/trip.schema.json`](schemas/trip.schema.json) | [`rest-api.md`](rest-api.md) §7.30 | The window-scoped live-sharing shapes (v0.41.0, MYR-602): `Trip` with its `TripStatus`/`TripRole` enums and the `TripVehicle`, `TripParticipant` and `TripCurrentLeg` projections, the `CreateTripRequest` and `UpdateTripRequest` bodies, the `TripListResponse` envelope this file is titled for, and `RegisterTripActivityRequest` — the per-user, per-trip push-to-start token that lets the SERVER open a driving leg's Live Activity while the app is backgrounded, which the per-activity ride registration cannot do. |
