...
- **Two later migrations, both P0 and neither adding data about a person ([MYR-612](https://linear.app/myrobotaxi/issue/MYR-612)).** `0049_trip_leg_resume` adds an INDEX only — `idx_go_trip_legs_vehicle_ended`, partial on closed legs — so that a leg wrongly closed by a transient destination clear can be RESUMED rather than duplicated; it stores nothing. `0050_trip_activity_token_leg_stamp` adds `go_trip_activity_tokens.started_leg_id`, an opaque leg cuid on a row that already names the trip and the party, claimed by both push-to-start senders so two Live Activities cannot be raised for one journey. **Neither changes the retention or deletion story:** the index has no rows of its own, and the column lives and dies with the row it sits on, which already cascades with its trip and is already removed at step 8g.
...
| `trip.participant_added` | Somebody was put on an EXISTING trip ([MYR-618](https://linear.app/myrobotaxi/issue/MYR-618), [`rest-api.md`](rest-api.md) §7.30.4). **IT EXISTS BECAUSE THE ACTOR IS NO LONGER CONSTANT:** until MYR-618 only a trip's owner could widen its roster, so *"who let this person see my car"* answered itself; a LIVE PARTICIPANT may now add anybody who already holds an accepted, unsuspended grant on the trip's vehicle, and the act therefore needs a record with a subject. Written by `store.TripRepo` **inside the roster transaction** (CG-DL-3), one row PER PERSON ACTUALLY ADDED — a re-send of somebody already aboard is a no-op and writes nothing — and **never on CREATE**, where the initial roster is part of the act of creating the trip and the attribution is the trip's own owner column. `targetType='trip'`, `targetId`=the trip's cuid, `userId`=**THE ACTOR** (owner or participant — the whole reason the row is worth writing), `initiator='user'`, `metadata={vehicleId, shareId}` — **three opaque cuids across the whole row and nothing else** (CG-DL-5). No trip NAME (P1 user content sealed at rest, §1.4.7) and no names of any kind. **⚠ THE ADDED PERSON'S USER ID IS DELIBERATELY ABSENT:** the SHARE id already names them to anybody entitled to ask this question — it is a row on the car's own grant list — while a user id would be a durable cross-surface identifier for a third party who is not the subject of the row. **DOTTED, like its `trip.deleted` and `vehicle.` neighbours** | User (the trip's owner, or a live participant) |
| `share.extended` | An owner copied one of their **accepted** share grants onto ANOTHER car they own ([MYR-609](https://linear.app/myrobotaxi/issue/MYR-609), [`rest-api.md`](rest-api.md) §7.5.8). Written by `store.VehicleShareRepo.ExtendShare` **inside the extend transaction and before the grant INSERT** (CG-DL-3). `targetType='vehicle'`, `targetId`=the car GAINING the grant, `userId`=the owner who acted, `initiator='user'`, `metadata={shareId, sourceShareId}` — **two opaque cuids and nothing else** (CG-DL-5). Deliberately **no `label`** and **no `code`** (both P1, [`data-classification.md`](data-classification.md) §1.15) and deliberately **not the GRANTEE's id**: the row is filed against the vehicle under the owner who acted, the two share ids resolve to the person for anybody holding the database, and an audit row is a place a value reaches permanent storage without anybody deciding it should. **IT IS THE ONLY RECORD THAT THIS ACCESS WAS NOT REDEEMED.** Every other accepted row in `go_vehicle_shares` got there because somebody presented a code; this one got there because an owner pressed a button, and the row itself cannot say so — an extended grant is byte-for-byte an ordinary accepted grant, which is the point, since every gate must treat it as one. So this is what answers "how did this person get access to this car when no invite for it was ever redeemed?". **DOTTED, like its `trip.` and `vehicle.` neighbours:** a share-scoped sub-action, not a platform lifecycle verb | User (the car's owner) |
...
| `trip` | A `go_trips` record — the window itself, paired with `action: trip.deleted` ([MYR-607](https://linear.app/myrobotaxi/issue/MYR-607), §3.6) or `action: trip.participant_added` ([MYR-618](https://linear.app/myrobotaxi/issue/MYR-618)). `targetId` is the trip's own cuid, unlike the two batch target types above it: these rows record ONE named window, not a group, so the id they name is the id that was acted on. **The trip stays the target even when the row is about a PERSON** — a participant add is a change to the window's membership, and keying it on the person added would make *"what happened to this trip"* a query nobody could write |
